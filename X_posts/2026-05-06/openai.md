# OpenAI X投稿 - 2026-05-06

収集元: ローカルRSSHub (localhost:1200)

---

## @OpenAIDevs (OpenAI Developers - 公式開発者アカウント)

**03:45 JST** | [原文](https://x.com/OpenAIDevs/status/2051735087637746162)

> http://x.com/i/article/2051727987352141824

---

**03:05 JST** | [原文](https://x.com/OpenAIDevs/status/2051725072873001338)

> The updated Agents SDK is now available in TypeScript, with support for sandbox agents and an open-source harness built in.
> 
> OpenAI Developers: Build long-running agents with more control over agent execution.
> 
> New capabilities in the Agents SDK:
> • Run agents in controlled sandboxes
> • Inspect and customize the open-source harness
> • Control when memories are created and where they’re stored

---

**00:54 JST** | [原文](https://x.com/OpenAIDevs/status/2051783570386227540)

> RT Michael Wall
> http://x.com/i/article/2051667653903998976

---

## @sama (Sam Altman - CEO)

**10:00 JST** | [原文](https://x.com/sama/status/2051829422265979047)

> ChatGPT feels very 'switched on' now

---

**06:57 JST** | [原文](https://x.com/sama/status/2051783339502375418)

> 5.5 in codex is so good for non-coding tasks.
> 
> i keep assuming it won't be able to do something, but a lot of the time i am pleasantly surprised.

---

**05:18 JST** | [原文](https://x.com/sama/status/2051758445402223051)

> in particular, the combination of improvements to speed, intelligence, personality, and great memory/personalization feels like a more-than-sum-of-the-parts thing when it all hits together
> 
> Sam Altman: the new instant model in chatgpt is so good damn
> 
> if you have been thinking-model-only for awhile, give it a try!

---

**05:17 JST** | [原文](https://x.com/sama/status/2051758152224506203)

> the new instant model in chatgpt is so good damn
> 
> if you have been thinking-model-only for awhile, give it a try!

---

**03:04 JST** | [原文](https://x.com/sama/status/2051724685231214650)

> i would like to talk to people who have built amazing things with 5.5 that weren't possible with earlier models. i am especially interested in examples that took ludicrous token budgets. thanks.

---

**02:33 JST** | [原文](https://x.com/sama/status/2051716909629153573)

> 5.5 instant comes to ChatGPT today!
> 
> imo it is a pretty big upgrade, i really like using it.
> 
> Eric: Excited that we're updating the default model in ChatGPT today!
> 
> 5.5 instant is a substantial improvement in intelligence, image perception, and factuality.
> 
> It also updates the writing style to be a bit plainer and more straightforward.
> 
> What was on your wishlist?

---

**02:04 JST** | [原文](https://x.com/sama/status/2051722309162160429)

> RT Michelle Pokrass
> we shipped gpt-5.5 instant today to chat; it's rolling out over the next couple days to everyone. for this model, we focused on factuality, crushing hacks, and improving the baseline intelligence. 5.5 is a pretty big step forward on all three. it is much smarter, significantly less likely to hallucinate, and just more delightful to talk to. there is always more to do but i'm pretty excited about this one!

---

**00:47 JST** | [原文](https://x.com/sama/status/2051690237420826838)

> hey chat, we haven't forgotten about you
> 
> 👀

---

## @gdb (Greg Brockman - 共同創業者)

**12:27 JST** | [原文](https://x.com/gdb/status/2051866486658994636)

> ChatGPT for Excel and Google Sheets:
> 
> ChatGPT: ChatGPT is now available as an add-on in Excel and Google Sheets.
> 
> It can help analyze messy data, write formulas, update spreadsheets, and explain what it’s doing along the way—without leaving your spreadsheet.
> 
> Powered by GPT-5.5.
> 
> https://chatgpt.com/apps/spreadsheets/

---

## @polynoamial (Noam Brown - 研究者)

**04:59 JST** | [原文](https://x.com/polynoamial/status/2051782906146943269)

> RT prinz
> I did not previously realize that GPT-5.5 is approximately 4x to 5x cheaper than Mythos.
> 
> Natalia Coelho's excellent analysis of Mythos' capabilities based on publicly available information (linked in the quoted tweet) includes the below graph regarding the two models' performances on UK AISI's TLO benchmark.
> 
> We already knew that GPT-5.5 performed slightly better on this benchmark than Mythos on average (although completely succeeding on it in only 2/10 attempts vs. 3/10 for Mythos), but it came as a surprise to me that GPT-5.5's attempts cost only ~$500 on average, compared to Mythos' ~$2.6K.
> 
> If you are a hacker, you will not, ofc, care about the number of tokens your attack used.  The only factor you *would* care about (maybe) is the cost.  And I venture to guess that spending 500M tokens on GPT-5.5 vs. 100M tokens on Mythos would likely be a no-brainer (GPT-5.5 would win).
> 
> From Noam Brown's tweet from a few days ago about GPT-5.5's performance on TLO (linked below):
> 
> "After 100 million tokens, performance was still going up. What we're seeing here is not the capability ceiling."
> 
> Tim Duffy: Good analysis of Mythos benchmark performance by @natalia__coelho, including interesting points on benchmark contamination, which I think is underrated. https://pointestimate.substack.com/p/how-good-is-mythos

---

