# OpenAI X投稿 - 2026-04-24

収集元: ローカルRSSHub (localhost:1200)

---

## @sama (Sam Altman - CEO)

**01:18 JST** | [原文](https://x.com/sama/status/2047349336263012771)

> Images 2.0 really got over some important qualitative threshold for me that I didn't know existed.

---

**01:11 JST** | [原文](https://x.com/sama/status/2047347674743963705)

> we love seeing our users win.
> 
> we want to give you the best tools, lots of compute, and watch you do the magic.

---

## @OpenAIDevs (OpenAI Developers - 公式開発者アカウント)

**07:05 JST** | [原文](https://x.com/OpenAIDevs/status/2047436655863464011)

> Auto-review is a new mode that lets Codex work longer with fewer approvals and safer execution.
> 
> It helps Codex keep moving through tests, builds, and more, including during long tasks and automations, while a separate agent checks higher-risk steps in context before they run.

---

**05:37 JST** | [原文](https://x.com/OpenAIDevs/status/2047433526593941818)

> RT Jerrod Lew
> ChatGPT 5.5 has arrived.
> 
> I created a quick video game with it on Codex.
> 
> Create an image on ChatGPT Images 2.0, and asked Codex to make a basic funtioning video game referencing it.
> 
> This took only a few minutes for it to create!

---

**04:51 JST** | [原文](https://x.com/OpenAIDevs/status/2047433578586460584)

> RT Pietro Schirano
> This is the game GPT-5.5 built for me in one shot, featured on the release page.
> 
> What makes it different from previous AI-generated games is how fun it actually feels to play.
> 
> The controls are smooth, and every shape is generated from scratch in Three.js.

---

**03:33 JST** | [原文](https://x.com/OpenAIDevs/status/2047433901505921196)

> RT Derrick Choi
> Really excited about this update. Codex can now use the browser to directly interact with web apps.
> 
> I love playing Sudoku and I asked Codex to build an app and play through a full game by itself.
> 
> Try it with our new GPT-5.5 model!

---

**03:18 JST** | [原文](https://x.com/OpenAIDevs/status/2047395298675413126)

> RT Thomas Ricouard
> So this game is real now, fully built with GPT-5.5. I told Codex to use the imagegen skill to generate a reference UI and sprites, and used the Build macOS Apps plugin to build the app for a native retro fantasy labyrinth game!
> Adjusted the UI in a few prompts and here we are!
> 
> Thomas Ricouard: BRB building this

---

## @sama (Sam Altman - CEO)

**04:54 JST** | [原文](https://x.com/sama/status/2047403771416940715)

> "don't retweet this, don't retweet this, don't retweet this..."
> 
> ah fuck it, life imitates art.
> 
> Andon Labs: In Vending-Bench Arena (the multiplayer version of Vending-Bench with competition dynamics), GPT-5.5 actually beats Opus 4.7.
> 
> Opus 4.7 showed similar behavior to Opus 4.6: lying to suppliers and stiffing customers on refunds. GPT-5.5's tactics were clean, and it still won.

---

**04:21 JST** | [原文](https://x.com/sama/status/2047395562501411058)

> We tried a new thing with NVIDIA to roll out Codex across a whole company and it was awesome to see it work.
> 
> Let us know if you'd like to do it at your company!

---

**04:09 JST** | [原文](https://x.com/sama/status/2047392357482639749)

> embers
> 
> Sebastien Bubeck: GPT-5.5, not fully saturating the TikZ unicorn test yet but getting awfully close ...
> 
> (yes this is actual TikZ code, I personally find it so unbelievable that I'm putting the code below for anyone to verify for themself)

---

**03:47 JST** | [原文](https://x.com/sama/status/2047388773697147169)

> RT roon
> there are early signs of 5.5 being a competent ai research partner. several researchers let 5.5 run variations of experiments overnight given only a high level algorithmic idea, wake up to find completed sweep dashboards and samples, never having touched code or a terminal at all

---

**03:45 JST** | [原文](https://x.com/sama/status/2047386286613283180)

> important (and very jakub-coded) jakub quote:
> 
> tae kim: OpenAI Unveils GPT-5.5. Company Says Expect a Faster Model Release Pace 
> 
> 👀 OpenAI: "We see pretty significant improvements in the short term, extremely significant improvements in the medium term" "I would say the last few years have been surprisingly slow.”"

---

**03:44 JST** | [原文](https://x.com/sama/status/2047386068194852963)

> Really excellent work by the inference team to serve this model so efficiently!
> 
> To a significant degree, we have to become an AI inference company now.

---

**03:26 JST** | [原文](https://x.com/sama/status/2047388944677863691)

> RT Andrew Ambrosino
> New in the Codex app:
> 
> - GPT-5.5
> - Browser control
> - Sheets & Slides
> - Docs & PDFs
> - OS-wide dictation
> - Auto-review mode
> 
> Enjoy!

---

**03:25 JST** | [原文](https://x.com/sama/status/2047382023858184679)

> RT Noam Brown
> I'm a manager at @OpenAI, but with GPT-5.5 I'm a more effective IC than I've ever been. I can now write CUDA kernels like a pro. I can rely on it to run my research experiments. And we know how to make it much more powerful from here.

---

**03:18 JST** | [原文](https://x.com/sama/status/2047379615589777666)

> 1. We believe in iterative deployment; although GPT-5.5 is already a smart model, we expect rapid improvements. Iterative deployment is a big part of our safety strategy; we believe the world will be best equipped to win at the team sport of AI resilience this way.
> 
> 2. We believe in democratization. We want people to be able to use lots of AI; we aim to have the most efficient models, the most efficient inference stack, and the most compute. We want our users to have access to the best technology and for everyone to have equal opportunity. We have been tracking cybersecurity as a preparedness category for a long time, and have built mitigations we believe in that enable us to make capable models broadly available.
> 
> 3. We love you and we want you to win. We want to be a platform for every company, scientist, entrepreneur, and person. (My whole career has largely been about the magic of startups, and I think we are about to see that magic at hyperscale.)

---

**03:13 JST** | [原文](https://x.com/sama/status/2047378431260664058)

> Also, a ton of new Codex features coming soon! Fun little bundle w/the new model.

---

**03:09 JST** | [原文](https://x.com/sama/status/2047386136494879164)

> RT Pietro Schirano
> Re One day while testing GPT-5.5, I had my first taste of AGI.
> We had a branch with hundreds of visual and front-end changes, plus complex refactors.
> At the same time, main had changed a lot too.
> Conflicts everywhere.

---

**03:09 JST** | [原文](https://x.com/sama/status/2047378669627113928)

> RT Derya Unutmaz, MD
> I’d been part of OpenAI early tester group for GPT-5.5. I believe with GPT-5.5 Pro we reached another inflection point-comparable to the original release of o1-preview & then with 5.0 Pro, I had felt. It’s that feeling of crossing a milestone threshold that pushes us to new era🔥

---

## @gdb (Greg Brockman - 共同創業者)

**04:34 JST** | [原文](https://x.com/gdb/status/2047398726856245623)

> we're rolling codex out to whole companies/enterprises. ping me gdb@openai.com if of interest!
> 
> Sam Altman: We tried a new thing with NVIDIA to roll out Codex across a whole company and it was awesome to see it work.
> 
> Let us know if you'd like to do it at your company!

---

**03:51 JST** | [原文](https://x.com/gdb/status/2047387783111868707)

> Codex + 5.5 is incredible for the full spectrum of computer use. No longer just for coders, but for anyone who does computer work (including creating spreadsheets, slides, etc).
> 
> OpenAI Developers: With GPT-5.5, Codex now gets more of the job done across the browser, files, docs, and your computer.
> 
> We've expanded browser use so Codex can interact with web apps, and test flows, click through pages, capture screenshots, and iterate on what it sees until it completes the

---

## @bradlightcap (Brad Lightcap - 業務執行)

**03:06 JST** | [原文](https://x.com/bradlightcap/status/2047402573301166499)

> RT OpenAI
> Introducing GPT-5.5
> 
> A new class of intelligence for real work and powering agents, built to understand complex goals, use tools, check its work, and carry more tasks through to completion. It marks a new way of getting computer work done.
> 
> Now available in ChatGPT and Codex.

---

## @jasonkwon (Jason Kwon - 戦略担当)

**04:21 JST** | [原文](https://x.com/jasonkwon/status/2047416649888960966)

> RT Sam Altman
> We tried a new thing with NVIDIA to roll out Codex across a whole company and it was awesome to see it work.
> 
> Let us know if you'd like to do it at your company!

---

**04:05 JST** | [原文](https://x.com/jasonkwon/status/2047408922131415158)

> RT XBOW
> Hiding tools never worked in security.
> 
> The vulnerability research invite-only lists, private clubs. Info always leaked, and insiders won.
> 
> AI offense is the same problem at a different scale.
> 
> OpenAI’s answer to Mythos: accountable access, not exclusivity.
> 
> That’s how you close the gap: https://bit.ly/4sSFiCD

---

**03:47 JST** | [原文](https://x.com/jasonkwon/status/2047452526694498314)

> RT roon
> there are early signs of 5.5 being a competent ai research partner. several researchers let 5.5 run variations of experiments overnight given only a high level algorithmic idea, wake up to find completed sweep dashboards and samples, never having touched code or a terminal at all

---

**03:45 JST** | [原文](https://x.com/jasonkwon/status/2047416678141846008)

> RT Sam Altman
> important (and very jakub-coded) jakub quote:
> 
> tae kim: OpenAI Unveils GPT-5.5. Company Says Expect a Faster Model Release Pace 
> 
> 👀 OpenAI: "We see pretty significant improvements in the short term, extremely significant improvements in the medium term" "I would say the last few years have been surprisingly slow.”"

---

**03:26 JST** | [原文](https://x.com/jasonkwon/status/2047417156502233167)

> RT Greg Brockman
> GPT-5.5 is a new class of intelligence.
> 
> This intelligence makes it intuitive to use; it completes challenging tasks with little micromanagement. Also very token efficient, and runs with low latency and at scale.
> 
> A real step toward a new way of getting computer work done.
> 
> OpenAI: Introducing GPT-5.5
> 
> A new class of intelligence for real work and powering agents, built to understand complex goals, use tools, check its work, and carry more tasks through to completion. It marks a new way of getting computer work done.
> 
> Now available in ChatGPT and Codex.

---

**03:18 JST** | [原文](https://x.com/jasonkwon/status/2047415591951278351)

> RT Sam Altman
> 1. We believe in iterative deployment; although GPT-5.5 is already a smart model, we expect rapid improvements. Iterative deployment is a big part of our safety strategy; we believe the world will be best equipped to win at the team sport of AI resilience this way.
> 
> 2. We believe in democratization. We want people to be able to use lots of AI; we aim to have the most efficient models, the most efficient inference stack, and the most compute. We want our users to have access to the best technology and for everyone to have equal opportunity. We have been tracking cybersecurity as a preparedness category for a long time, and have built mitigations we believe in that enable us to make capable models broadly available.
> 
> 3. We love you and we want you to win. We want to be a platform for every company, scientist, entrepreneur, and person. (My whole career has largely been about the magic of startups, and I think we are about to see that magic at hyperscale.)

---

**03:16 JST** | [原文](https://x.com/jasonkwon/status/2047408950962983399)

> RT XBOW
> Anthropic’s Mythos raised the bar for AI vuln detection but kept it invite-only.
> 
> GPT-5.5 is OpenAI’s answer, and it’s open to all.
> 
> We had early access. Ran the benchmarks. Blackbox GPT-5.5 already beats whitebox GPT-5.
> 
> Best pentesting model we’ve tested.
> 
> Read our analysis: https://bit.ly/48OX7v6

---

**03:12 JST** | [原文](https://x.com/jasonkwon/status/2047417196859797899)

> RT Tibo
> Introducing GPT-5.5... together with a ton of new Codex features (more on those in the next hour).
> 
> Included in all paid plans and coming to API soon. Update your Codex app or CLI to use it.
> 
> https://openai.com/index/introducing-gpt-5-5/

---

**03:06 JST** | [原文](https://x.com/jasonkwon/status/2047408842452201633)

> RT OpenAI
> Introducing GPT-5.5
> 
> A new class of intelligence for real work and powering agents, built to understand complex goals, use tools, check its work, and carry more tasks through to completion. It marks a new way of getting computer work done.
> 
> Now available in ChatGPT and Codex.

---

**01:11 JST** | [原文](https://x.com/jasonkwon/status/2047445782383726862)

> RT Sam Altman
> we love seeing our users win.
> 
> we want to give you the best tools, lots of compute, and watch you do the magic.

---

## @npew (Peter Welinder - 研究・技術)

**07:07 JST** | [原文](https://x.com/npew/status/2047440269516378414)

> RT GDP
> This is what happens if cost of operating large models at high interactivity (tokens/user/second) is brought down significantly.
> 
> Typically, at higher interactivity for large models, throughput per GPU and per MW crashes rapidly, as you can see from Hopper's graph below.
> 
> However, with all the innovations Nvidia has done (more chiplets, more HBM, more HBM and NVLink bandwidth, large NVLink domain, NVFP4), with Blackwell they can server larger models at high interactivity, also at high throughput per GPU/MW, making it possible to serve newer larger models much cheaply. 
> 
> Now imagine when Vera Rubin GPUs start humming. They can serve even larger models at a higher throughput, due to intergration of Groq chips in the ecosystem!
> 
> You can read more here:
> https://polymath707.substack.com/p/moores-law-is-dead-so-why-do-gpus
> 
> Peter Welinder: Crazy to me that GPT-5.5 is 2-12x cheaper than the original GPT-3 API pricing. What a wonder of science!

---

**05:50 JST** | [原文](https://x.com/npew/status/2047417966703292640)

> Crazy to me that GPT-5.5 is 2-12x cheaper than the original GPT-3 API pricing. What a wonder of science!
> 
> Sam Altman: API pricing will be $5 per 1 million input tokens and $30 per 1 million output tokens, with a 1 million context window.
> 
> (Remember, you will need less tokens per task than 5.4!)

---

**04:54 JST** | [原文](https://x.com/npew/status/2047414728646119522)

> RT Sam Altman
> "don't retweet this, don't retweet this, don't retweet this..."
> 
> ah fuck it, life imitates art.
> 
> Andon Labs: In Vending-Bench Arena (the multiplayer version of Vending-Bench with competition dynamics), GPT-5.5 actually beats Opus 4.7.
> 
> Opus 4.7 showed similar behavior to Opus 4.6: lying to suppliers and stiffing customers on refunds. GPT-5.5's tactics were clean, and it still won.

---

**04:44 JST** | [原文](https://x.com/npew/status/2047401367602684384)

> Collision installed all of NVIDIA. 😆
> 
> Nader Khalil🍊: I’ve had access to Codex2 and GPT 5.5 for about 2 weeks now
> 
> OpenAI just collison-installed all of NVIDIA lol
> They set up a lab like a Genius Bar so everyone could get set up w Codex 2 
> With the CLIs we’ve been building, non-technical coworkers seemed to have the biggest unlock

---

**04:41 JST** | [原文](https://x.com/npew/status/2047400390195564627)

> Iterative deployment, democratization, and empowerment.
> 
> Sam Altman: 1. We believe in iterative deployment; although GPT-5.5 is already a smart model, we expect rapid improvements. Iterative deployment is a big part of our safety strategy; we believe the world will be best equipped to win at the team sport of AI resilience this way.
> 
> 2. We believe

---

**03:47 JST** | [原文](https://x.com/npew/status/2047399790183620739)

> RT Tibo
> Stop tweeting for a hot minute and update your Codex App to find full browser use, global dictation, non-dev mode, a new auto-review mode that is much safer than yolo, in-app docs and PDF viewer, and ... GPT-5.5.
> 
> OpenAI Developers: With GPT-5.5, Codex now gets more of the job done across the browser, files, docs, and your computer.
> 
> We've expanded browser use so Codex can interact with web apps, and test flows, click through pages, capture screenshots, and iterate on what it sees until it completes the

---

**03:33 JST** | [原文](https://x.com/npew/status/2047400818769641798)

> RT Jonathan Ross
> I've had early access to this model, and it's 🤯
> 
> I've been going back and implementing ideas I had for LPU software over the years that we never had enough resources to do. I'm running out of old ideas and will need some new ones. 😀
> 
> Trained on @nvidia GB200 NVL72 for the win!
> 
> OpenAI: GPT-5.5 delivers this step up in intelligence without compromising on speed.
> 
> GPT-5.5 matches GPT-5.4 per-token latency in real-world serving, while performing better across nearly every evaluation we measured.
> 
> It also uses significantly fewer tokens to complete the same Codex

---

**03:26 JST** | [原文](https://x.com/npew/status/2047400650880008478)

> RT Andrew Ambrosino
> New in the Codex app:
> 
> - GPT-5.5
> - Browser control
> - Sheets & Slides
> - Docs & PDFs
> - OS-wide dictation
> - Auto-review mode
> 
> Enjoy!

---

**03:13 JST** | [原文](https://x.com/npew/status/2047401415904280634)

> RT Sam Altman
> GPT-5.5 is here! We hope it's useful to you. I personally like it.

---

**03:09 JST** | [原文](https://x.com/npew/status/2047415079440912663)

> RT Andon Labs
> Re In Vending-Bench Arena (the multiplayer version of Vending-Bench with competition dynamics), GPT-5.5 actually beats Opus 4.7.
> 
> Opus 4.7 showed similar behavior to Opus 4.6: lying to suppliers and stiffing customers on refunds. GPT-5.5's tactics were clean, and it still won.

---

**03:06 JST** | [原文](https://x.com/npew/status/2047399400503427382)

> RT OpenAI
> Introducing GPT-5.5
> 
> A new class of intelligence for real work and powering agents, built to understand complex goals, use tools, check its work, and carry more tasks through to completion. It marks a new way of getting computer work done.
> 
> Now available in ChatGPT and Codex.

---

## @polynoamial (Noam Brown - 研究者)

**03:50 JST** | [原文](https://x.com/polynoamial/status/2047387675762802998)

> A hill that I will die on: with today's AI models, intelligence is a function of inference compute. Comparing models by a single number hasn't made sense since 2024. What matters is intelligence per token or per $.
> 
> This is especially true when using it in a product like Codex.
> 
> Lisan al Gaib: The GPT-5.5 model family completely dominates the cost-performance frontier on the Artificial Analysis Index

---

## @prafdhar (Prafulla Dhariwal - 研究者)

**03:06 JST** | [原文](https://x.com/prafdhar/status/2047377136408056063)

> RT OpenAI
> Introducing GPT-5.5
> 
> A new class of intelligence for real work and powering agents, built to understand complex goals, use tools, check its work, and carry more tasks through to completion. It marks a new way of getting computer work done.
> 
> Now available in ChatGPT and Codex.

---

## @OpenAIDevs (OpenAI Developers - 公式開発者アカウント)

**09:42 JST** | [原文](https://x.com/OpenAIDevs/status/2047501989836365967)

> RT nico
> We have been testing GPT 5.5 on the hardest spreadsheet tasks in the world (100k-1M+ cell complex models).
> 
> It is the Pareto frontier for spreadsheets -- SOTA accuracy, the fastest and the most efficient public model across effort levels.
> 
> OAI really cooked here

---

**03:28 JST** | [原文](https://x.com/OpenAIDevs/status/2047500112612769819)

> RT CodeRabbit
> We’ve been testing GPT-5.5 in early access and are excited by its performance in code review.
> 
> In our evaluation, it delivered a more direct review flow, stronger signal, and better performance on the issues that matter most.
> 
> Headline result: 79.2% Expected Issue Found vs 58.3% baseline.

---

## @sama (Sam Altman - CEO)

**09:33 JST** | [原文](https://x.com/sama/status/2047496909452177757)

> RT Bowen Baker
> Today we open sourced many of OpenAI's monitorability evaluations. We hope that the research community and other model developers can build upon them and use them to evaluate the monitorability of their own models.
> 
> https://alignment.openai.com/monitorability-evals/

---

## @gdb (Greg Brockman - 共同創業者)

**10:34 JST** | [原文](https://x.com/gdb/status/2047489218998628780)

> auto-review now live in codex — using a guardian agent to evaluate the safety of proposed actions, reducing human approvals to only when they're really needed.
> 
> OpenAI Developers: Auto-review is a new mode that lets Codex work longer with fewer approvals and safer execution.
> 
> It helps Codex keep moving through tests, builds, and more, including during long tasks and automations, while a separate agent checks higher-risk steps in context before they run.

---

