...
10. テキスト出力なら `references/38-style-readable-breaks.md` を読む。
11. 必要に応じて theory module を読む（`40-` 系）。
12. 実行前に `references/33-prompt-assembly.md` の方式で、選んだモジュールだけを束ねる。
13. 出力は一発で完成させず、複数パスで作る。
...
3. **draft**: 本文を作る。
4. **diverge**: 平均的な語り口、安いまとめ、無難な中間結論を壊す。
5. **gate**: rubric で検査し、さらに読み込んだ全モジュール（style overlay、audience module、task module）の最終チェック・quality gate・failure modes も検証する。欠点があれば直す。
