...
文脈は期限順20予測、種類ごと直近40件を基本に制限します。企業の節更新は重複履歴を渡さず、最新の全本文と節のIDを渡します。過去Informationは会社ごと直近3件の探索候補を追加します。全履歴は保存されますが、この範囲外もレビュー済みとは扱いません。文脈上限はUTF-8で400,000バイトです。台帳と旧資料検索は現在全件を読みます。規模が増えた際は、原本と照合できる索引を追加する必要があります。
