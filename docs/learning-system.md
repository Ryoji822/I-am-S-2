...
| DossierSection | 企業・市場の節の更新全文、解釈、限界、根拠、確認時刻、直前の節更新ID |
...
1. 重大な危険、期限、前回の反証、基準不足の順に原資料を最大5件、検索を最大2回まで確認し、最大20件の根拠記録を作る。残りの確認範囲は欠測として明記する。
...
6. 保存された企業・市場の全本文と記録から日報・Static・成績・定期レビューを表示する。

企業と市場の基礎本文は`config/dossiers/`の7冊です。更新は`dossier_section`として反証・裁定を通し、対象節の全文だけを置き換えます。他の節は保持します。新着のない日・部分収集日にも企業情報を省きません。事実とその出典、解釈、残る限界を区別し、未知の対象節、未検証の新規URL、未来の根拠、履歴の分岐を保存前に拒否します。機械的な検査は主張の真偽や全ての情報落ちを証明しないため、反証段階で更新前の有効な情報の維持も点検します。

実行の`accepted-plan.json`には採用した全本文も固定します。表示失敗からの再開は、この本文を使い、後の基礎資料の変更やモデルの再呼出しに依存しません。shadowのStaticも`state/shadow/static_intelligence/`へ出します。元資料の基準日と更新した節の確認時刻を併記します。
...
収集がpartialなら根拠・実測と決定的な判定だけを保存できます。判断を作る3段階のモデル呼出しは省き、企業本文を維持します。原資料の再取得でHTTPエラーやタイムアウトが起きた場合、その根拠と依存する実測を除き、取得済み資料が残ればpartialとして欠測を記録します。全資料が取得できなければfailedです。不正なURLや記録の検証エラーは欠測に読み替えません。failed/staleでは新しい判断を保存しません。前日資料のコピー、未反証の主分析の代用、検証失敗後の公開を止めます。警戒は資料不足で下げません。
...
文脈は期限順20予測、種類ごと直近40件を基本に制限します。企業の節更新は重複履歴を渡さず、分析・反証・裁定には最新の全本文と節のIDを渡します。収集には企業概要・節名・出典URLの索引を渡し、原資料を読む余地を残します。過去Informationは会社ごと直近3件の探索候補を追加します。全履歴は保存されますが、この範囲外もレビュー済みとは扱いません。文脈上限はUTF-8で400,000バイトです。文脈は標準入力で渡し、添付ファイルの行単位切り詰めを回避します。自動要約による出力形式の崩れを防ぐためOpenCodeの自動compactionは無効です。各段階にはフィールド名だけでなく型・必須項目・許容値を含む記録Schemaを渡します。台帳と旧資料検索は現在全件を読みます。規模が増えた際は、原本と照合できる索引を追加する必要があります。
...
`python3 scripts/search-information.py QUERY --company openai --before YYYY-MM-DD --after YYYY-MM-DD`で、会社・収集日・旧ID・出典を検索できます。読み取り専用のアダプターが元ファイルと行番号、旧INFO/EVD ID、取得日時、ハッシュを返します。日次処理もこれを探索候補として読みます。公開日時はnull、信頼区分はlegacy_unverifiedで、古いofficial等の分類を保証に使いません。取得日を発生日にせず、原資料を再確認してから別のEvidenceとして記録します。結果件数は独立事象数ではなく、件数上限と不正レコードの場所も返します。
...
収集・検証に失敗してその日の日報がまだない場合は、過去の確認済み状態を持ち越した参考資料を作ります。先頭に失敗と新しい判断を含まないことを明記し、既存日報やStaticは上書きしません。failedの実行を成功として自動公開せず、GitHub Actionsでは参考資料も実行artifactへ含めます。
