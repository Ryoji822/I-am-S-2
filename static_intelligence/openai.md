...
2026年4月23日、OpenAIは**Workspace Agents**を発表した [INFO-021](../Information/2026-04-23/collected-raw.md#INFO-021)。ChatGPT上で共有エージェントを構築し、チーム間の複雑なタスクと長時間ワークフローを処理する。Cloud-hosted Codex harness上で動作し、Slack連携、定期タスク、ツール統合を備える [INFO-022](../Information/2026-04-23/collected-raw.md#INFO-022)。Codex LabsがB2Bチャネルを開いたなら、Workspace Agentsはそのチャネルを通じて配布する製品の中身を変える——個人開発者向けからチーム全体のワークフロー自動化への転換だ。
...
- **本社**: サンフランシスコ
- **CEO**: Sam Altman
- **主力製品**: ChatGPT、GPT-5.4シリーズ（Pro/mini/nano）、Agents SDK、Frontierプラットフォーム、**Codex**（desktop control・browser・memory・90+ plugins・heartbeats・multi-terminal・SSH・画像生成）、**Codex Labs**（エンタープライズハンズオン）、**Workspace Agents**（共有エージェント・Slack連携・定期タスク・Codex harness）、Operator（CUA）、GPT-5.4-Cyber（サイバー防御特化）、**GPT-Rosalind**（ライフサイエンス特化）
...
Workspace AgentsはCloud-hosted Codex harness上で動作する共有エージェント。Slack連携、定期タスク、ツール統合を備え、チーム間の長時間ワークフローを処理する。GSI 7社のチャネルに「チーム全体を動かす製品」が載ることで、Codex Labsが開いたB2Bルートの価値が一段と上がる [Arbiter v3.58](../state/arbiter-2026-04-23.md)。
...
| [IND-013](../config/indicators.json) セキュリティ侵害頻度 | high | rising | Codex desktop control新攻撃表面。97%企業インシデント予期。Reset to Zero問題。**Comment and Control**でCopilot Agent API鍵漏洩確認 [INFO-008](../Information/2026-04-23/collected-raw.md#INFO-008) |
| [IND-026](../config/indicators.json) エージェント本番環境到達率 | elevated | rising | Workspace Agents + Codex Labs GSI 7社。Stanford 77%成功率 vs 54%計画/25%実績ギャップ。Reset to Zero。Comment and Control 3社同時脆弱性 |

## 変更履歴（直近5件のみ。詳細は git log を参照）

| 日付 | 変更内容 |
|------|---------|
| 2026-04-23 | Workspace Agents（共有エージェント・Slack連携・定期タスク・Codex harness）新製品発表・30GW compute by 2030計画・Comment and Control脆弱性（Copilot Agent API鍵漏洩）を反映して戦略方向性・SWOT・I&Wを書き直し。H-OAI-001 62→63%に更新 |
