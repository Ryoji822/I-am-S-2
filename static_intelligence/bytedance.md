...
ByteDanceのAI事業の現構図は、規模拡大が継続し、製品ポートフォリオが急速に拡充され、投資が加速する、という局面にある。前回「規模は確認済みだが経済モデルが試されている」と読んでいたが、5月14日時点では「規模拡大継続・製品ポートフォリオ急拡充・CAPEX増額で投資加速」と評価を更新する。

規模の根拠は強化された。Doubao MAU 3.45億・DAU 1.4億・月間平均54.8回利用で、QianwenとDeepSeekの合計MAUを上回る [INFO-063](../Information/2026-05-14/collected-raw.md#INFO-063)。中国AIアプリ首位の座は数字として確定している。
...
| 高 | ByteDance AI CAPEX 25%増額で2000億元(~$30B)。国内AIチップ投資比率拡大 | 投資加速の明確な兆候。規模成長を支える資本的裏付け | B-3 | [INFO-065](../Information/2026-05-14/collected-raw.md#INFO-065) |
| 高 | Seedance 2.0: Video Arena世界#2(ELO 1,271)・4モダリティ同時入力・World Model方向 | 製品品質の国際的第三者指標での確認。H-BTD-001の4x Cの1つ | B-3 (Video Arenaは独立検証) | [INFO-039](../Information/2026-05-14/collected-raw.md#INFO-039) |
...
- ByteDanceグローバルAI戦略への中国共産党の介入度が見えない。TikTok問題では政府介入が報道されたが、AI部門(Doubao/Seed)への介入の実態は公開情報にない。介入があれば製品ロードマップや人材配置が変わる。観測手段を持っていない。
- Seedモデルの学習データに含まれる検閲フィルタの実態が不明。中国国内で運用されるモデルが何をフィルタしているかは公開されていない。グローバル展開時に同一モデルを使う場合、西側市場での規制適合と中国側フィルタが矛盾する可能性がある。
- 米中規制摩擦(CHIPS法・Entity List)でのコンピュート調達制約の実態が不明。ByteDanceが高性能GPUにどこまでアクセスできているかは非公開。国内GPU(Ascend/Biren)への切り替え進捗も外部から確認できない。CAPEX 2000億元のうち国内AIチップ比率の具体値も不明。
- DeerFlow OSS・OpenClaw・TikTok MCPの海外での実際の採用状況が不明。発表段階の製品であり、ユーザー定着率・収益寄与・中国国外での利用実績は確認できない。C-3(第三者間接情報)に依存する。
- ミラーイメージングリスクが継続している。中国市場での成功(Doubao MAU 3.45億・4x C)を根拠にグローバル展開を推論しているが、これは分析者の前提を対象に投影する可能性がある。グローバル展開の直接的証拠が観測されるまで、この警告は継続する。
