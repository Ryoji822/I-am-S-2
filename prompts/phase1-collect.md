...
- **キーファクト:**
  ...
  - [事実2]
- **引用URL:** [原文URL]
- **Evidence ID:** EVD-YYYYMMDD-NNNN
```

**Evidence ID の付与ルール（必須）:**
- フォーマットは `EVD-{今日の日付YYYYMMDD}-{INFO番号を4桁ゼロ埋め}`
- 例: 今日が 2026-05-08 で `INFO-001` なら `EVD-20260508-0001`、`INFO-042` なら `EVD-20260508-0042`
- INFO番号と完全に一致させる（Phase 1.6 が同じ規則で `evidence_index.json` を生成するので、ここでズレると後段の参照整合検査で落ちる）
- **既存の INFO-XXX 体裁は絶対に変えない**。Evidence ID は **追加行** であり置き換えではない。
...
- scrape実行数: N件
- 収集情報数: N件
- Evidence ID 採番範囲: EVD-YYYYMMDD-0001 〜 EVD-YYYYMMDD-NNNN
...
タイムアウトの主因は「ファイル状態を見失ったままRead/Editを繰り返すこと」です。以下を守ること:

1. **追記は必ず末尾append**: `collected-raw.md` への追記は `Edit` ではなく、append機能（`>>` リダイレクト相当）または **直前に末尾内容をReadして得た文字列を `oldString` にしたEdit** のいずれかで行う。**過去に書いた箇所の `oldString` を記憶ベースで打たないこと**（既に上書きされている可能性が高い）。
2. **Readのoffsetは推測で打たない**: `offset` を指定するなら直前に `wc -l` 相当でファイル行数を確認してから。**範囲外オフセットを再試行しないこと**（exit 124の主因）。
3. **Edit失敗時の自動リカバリ**: `Could not find oldString` または `Offset out of range` が出たら、**同じ操作をリトライせず**に「ファイル末尾の最終50行をRead」→「現状を踏まえて新しいoldStringで再構築」へ切り替えること。
4. **同じツールを3回連続失敗したら諦めて次のKIQへ進む**: 1つのINFOエントリのために5分以上ツール失敗を続けない。
...
- **同じEdit/Readを範囲外パラメータで繰り返すこと**（タイムアウト誘発）
